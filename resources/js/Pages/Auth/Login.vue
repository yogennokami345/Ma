...
// Defina a URL da imagem via variável computada
const gridImgUrl = computed(() => `${import.meta.env.BASE_URL}images/grid.svg`);

function submit() {
  router.post(route('loginApi'), {
    email: email.value,
    password: password.value
  }, {
    onSuccess: () => {
      toast.success('Login realizado com sucesso!', { theme: 'dark', dangerouslyHTMLString: true })
    },
    onError: (errors) => {
              ...
              <!-- <a :href="route('discordAuth')">
                ...
                <form class="flex flex-col gap-5 items-center" action="#" @submit.prevent="submit">
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      ...
                      <Link :href="route('register')" class="font-medium text-primary-600 hover:underline dark:text-primary-500">
                        Criar conta
                      </Link>
                  </p>
                </form>
